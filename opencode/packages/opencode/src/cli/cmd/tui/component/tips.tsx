...
const TIPS = [
  ...
  "Create {highlight}drc.json{/highlight} in project root for project-specific settings",
  "Place settings in {highlight}~/.config/drc/drc.json{/highlight} for global config",
  ...
  "DrC auto-handles OAuth for remote MCP servers requiring auth",
  "Add {highlight}.md{/highlight} files to {highlight}.drc/command/{/highlight} to define reusable custom prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks in commands to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.drc/agent/{/highlight} for specialized AI personas",
  "Configure per-agent permissions for {highlight}edit{/highlight}, {highlight}bash{/highlight}, and {highlight}webfetch{/highlight} tools",
  'Use patterns like {highlight}"git *": "allow"{/highlight} for granular bash permissions',
  'Set {highlight}"rm -rf *": "deny"{/highlight} to block destructive commands',
  'Configure {highlight}"git push": "ask"{/highlight} to require approval before pushing',
  "DrC auto-formats files using prettier, gofmt, ruff, and more",
  'Set {highlight}"formatter": false{/highlight} in config to disable all auto-formatting',
  "Define custom formatter commands with file extensions in config",
  "DrC uses LSP servers for intelligent code analysis",
  "Create {highlight}.ts{/highlight} files in {highlight}.drc/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.drc/plugin/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent DrC from reading sensitive files",
  "Use {highlight}drc run{/highlight} for non-interactive scripting",
  "Use {highlight}drc --continue{/highlight} to resume the last session",
  "Use {highlight}drc run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}drc serve{/highlight} for headless API access to DrC",
  "Use {highlight}drc run --attach{/highlight} to connect to a running server",
  "Run {highlight}drc upgrade{/highlight} to update to the latest version",
  "Run {highlight}drc auth list{/highlight} to see all configured providers",
  "Run {highlight}drc agent create{/highlight} for guided agent creation",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.drc/themes/{/highlight} directory",
  ...
  "Run {highlight}drc debug config{/highlight} to troubleshoot configuration",
  ...
  "DrC specializes in Csound synthesis and Cabbage UI development",
  "Use {highlight}csound_compile{/highlight} to verify your .csd files compile correctly",
  "DrC validates widget/channel bindings in your Cabbage instruments",
]
