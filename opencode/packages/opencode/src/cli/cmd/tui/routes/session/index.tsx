...
export function Session() {
  ...
  createEffect(() => {
    ...
    return exit.message.set(
      [
        ...
        `  █  █  ${UI.Style.TEXT_DIM}drc -s ${session()?.id}${UI.Style.TEXT_NORMAL}`,
        `  ▀▀▀▀  `,
      ].join("\n"),
    )
  })
  ...
  command.register(() => [
    ...
    {
      title: csdPanelActive() ? "Hide CSD panel" : "Show CSD panel",
      value: "session.csd_panel.toggle",
      category: "Csound",
    ...
    },
    {
      ...
      value: "session.workspace.discard",
      category: "Csound",
      enabled: SessionWorkspace.isActive(route.sessionID),
      onSelect: async (dialog) => {
        try {
          await SessionWorkspace.discard(route.sessionID)
          toast.show({
            message: "Workspace changes discarded",
            variant: "success",
          })
        } catch (err) {
          toast.show({
            message: `Discard failed: ${String(err)}`,
            variant: "error",
          })
        }
        dialog.clear()
    ...
    {
      title: "Rate retrieval: helpful",
      value: "session.retrieval.thumbsup",
      category: "Csound",
      enabled: !!RetrievalEngine.getLastRetrieval(route.sessionID),
      onSelect: async (dialog) => {
        const retrieval = RetrievalEngine.getLastRetrieval(route.sessionID)
        if (!retrieval) {
          toast.show({ message: "No recent retrieval to rate", variant: "error" })
          dialog.clear()
          return
        }
        await RetrievalFeedback.record(route.sessionID, "", retrieval.chunkIDs, "user_thumbs_up")
        const sections = retrieval.results.map((r) => r.section).slice(0, 3).join(", ")
        toast.show({
          message: `Marked ${retrieval.chunkIDs.length} chunks as helpful (${sections})`,
          variant: "success",
        })
        dialog.clear()
      },
    ...
    {
      title: "Rate retrieval: not helpful",
      value: "session.retrieval.thumbsdown",
      category: "Csound",
      enabled: !!RetrievalEngine.getLastRetrieval(route.sessionID),
      onSelect: async (dialog) => {
        const retrieval = RetrievalEngine.getLastRetrieval(route.sessionID)
        ...
        await RetrievalFeedback.record(route.sessionID, "", retrieval.chunkIDs, "user_thumbs_down")
        const sections = retrieval.results.map((r) => r.section).slice(0, 3).join(", ")
        toast.show({
          message: `Marked ${retrieval.chunkIDs.length} chunks as not helpful (${sections})`,
          variant: "success",
        })
        dialog.clear()
      },
    ...
    {
      title: `Lock parameters (${lockedParams().size} locked)`,
      value: "session.lock.params",
      category: "Csound",
      enabled: designMode() && !!csdFilePath(),
      onSelect: async (dialog) => {
        ...
        try {
          ...
          dialog.replace(() => (
            <DialogLockParams
              ...
              onSave={(locked) => {
                setLockedParamsRaw([...locked].join(","))
                toast.show({
                  message: `${locked.size} parameter(s) locked`,
                  variant: "success",
                })
                dialog.clear()
              }}
  ...
  return (
    <Show when={props.part.text.trim()}>
      <box id={"text-" + props.part.id} paddingLeft={3} marginTop={1} flexShrink={0}>
        <Switch>
          <Match when={Flag.DRC_EXPERIMENTAL_MARKDOWN}>
            <markdown
              syntaxStyle={syntax()}
              streaming={true}
...
function ApplyPatch(props: ToolProps<typeof ApplyPatchTool>) {
  ...
  const allCsd = createMemo(() => files().length > 0 && files().every((f) => f.filePath?.endsWith(".csd")))

  // Compute semantic CSD changes from diff
  const csdChanges = createMemo(() => {
    if (!allCsd()) return []
    try {
      const output = props.output ?? ""
      // Extract old/new content from diff hunks to detect semantic changes
      const changes: CsdChangeEntry[] = []
      for (const file of files()) {
        if (!file.diff || !file.filePath?.endsWith(".csd")) continue
        const diffChanges = parseDiffForChanges(file.diff)
      ...
      <Match when={allCsd()}>
        <box flexDirection="column">
          <InlineTool icon="%" pending="Preparing patch..." complete={true} part={props.part}>
            Patched {files().map((f) => f.relativePath || normalizePath(f.filePath)).join(", ")}
          </InlineTool>
          <Show when={csdChanges().length > 0}>
            <box flexDirection="column" paddingLeft={2}>
              <For each={csdChanges().slice(0, 6)}>
                {(change) => (
                  <text>
                    <span style={{ fg: change.color, bold: true }}>{change.icon}</span>{" "}
                    <span style={{ fg: theme.text }}>{change.detail}</span>
                  </text>
                )}
              </For>
              <Show when={csdChanges().length > 6}>
                <text fg={theme.textMuted}>...and {csdChanges().length - 6} more</text>
              </Show>
            </box>
          </Show>
        </box>
...
// --- CSD Change Summary Helpers ---

interface CsdChangeEntry {
  icon: string
  color: string
  detail: string
}
...
const CSD_OPCODE_RE =
  /\b(oscili?|poscil|vco2|moogladder|lpf18|zdf_\w+|butter\w+|reverbsc|reverb2?|freeverb|nreverb|delay[rw]?|deltap[i3]?|flanger|chorus|phaser[12]|distort|tanh|powershape|clip|wrap|fold|loscil[3]?|diskin2?|foscili?|buzz|gbuzz|grain3?|partikkel|fog|fof2?|wg\w+|repluck|pluck|noise|pinker?|linen|adsr|madsr|mxadsr|envlpx|linseg|expseg|transeg|cosseg|loopseg|jspline|rspline|port[k]?|tone[kx]?|atone[kx]?|reson[x]?|areson|bqrez|svfilter|statevar|mode|compress2?|dam|follow2?|rms|balance2?|pan2?|vbap[48]?|hrtf\w+|pvs\w+|lfo|phasor|metro|schedule|schedkwhen|ftgen)\b/gi

function parseDiffForChanges(diff: string): CsdChangeEntry[] {
  const changes: CsdChangeEntry[] = []
  const added: string[] = []
  ...
  for (const line of added) {
    const instrMatch = line.trim().match(/^instr\s+(.+)/)
    if (instrMatch) {
      changes.push({ icon: "+", color: "green", detail: `instr ${instrMatch[1].trim()}` })
    }
  }
  ...
  for (const line of removed) {
    const instrMatch = line.trim().match(/^instr\s+(.+)/)
    if (instrMatch) {
      changes.push({ icon: "-", color: "red", detail: `instr ${instrMatch[1].trim()}` })
    }
  }

  // Detect added opcodes
  const addedOpcodes = new Set<string>()
  const removedOpcodes = new Set<string>()
  for (const line of added) {
    for (const m of line.match(CSD_OPCODE_RE) ?? []) addedOpcodes.add(m.toLowerCase())
  }
  for (const line of removed) {
    for (const m of line.match(CSD_OPCODE_RE) ?? []) removedOpcodes.add(m.toLowerCase())
  }
  // Only show net additions/removals
  for (const op of addedOpcodes) {
    if (!removedOpcodes.has(op)) {
      changes.push({ icon: "+", color: "green", detail: op })
    }
  }
  for (const op of removedOpcodes) {
    if (!addedOpcodes.has(op)) {
      changes.push({ icon: "-", color: "red", detail: op })
    }
  }

  // Detect parameter value changes (kVar = value or init value patterns)
  for (const addLine of added) {
    const paramMatch = addLine.trim().match(/^([ikaS]\w+)\s*(?:=|init)\s*([\d.eE+-]+)/)
    if (!paramMatch) continue
    const [, name, newVal] = paramMatch
    for (const remLine of removed) {
      const oldMatch = remLine.trim().match(new RegExp(`^${name.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\s*(?:=|init)\\s*([\\d.eE+-]+)`))
      if (oldMatch) {
        changes.push({ icon: "~", color: "yellow", detail: `${name} ${oldMatch[1]} -> ${newVal}` })
        break
      }
    }
  }

  // Detect new function tables
  for (const line of added) {
    const ftMatch = line.trim().match(/\b(gi\w+)\s+ftgen/)
    if (ftMatch) {
      changes.push({ icon: "+", color: "green", detail: ftMatch[1] })
    }
  }
