...
import { EOL } from "os"
import { createOpencodeClient, type Message, type OpencodeClient, type ToolPart } from "@drc/sdk/v2"
...
export const RunCommand = cmd({
  ...
  handler: async (args) => {
    ...
    async function share(sdk: OpencodeClient, sessionID: string) {
      ...
      if (!cfg.data) return
      if (cfg.data.share !== "auto" && !Flag.DRC_AUTO_SHARE && !args.share) return
