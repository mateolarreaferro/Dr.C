...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run DrC",
  builder: (yargs) =>
  ...
  async handler(args) {
    await Instance.provide({
      ...
      async fn() {
          ...
          if (prInfoText.trim()) {
            ...
            if (prInfo && prInfo.body) {
              ...
              if (sessionMatch) {
                ...
                UI.println(`Found DrC session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await $`drc import ${sessionUrl}`.nothrow()
        ...
        UI.println()
        UI.println("Starting DrC...")
        UI.println()

        // Launch opencode TUI with session ID if available
        const { spawn } = await import("child_process")
        const opencodeArgs = sessionId ? ["-s", sessionId] : []
        const opencodeProcess = spawn("drc", opencodeArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })
        ...
        await new Promise<void>((resolve, reject) => {
          opencodeProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`drc exited with code ${code}`))
          })
