...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall drc and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g drc",
      pnpm: "pnpm uninstall -g drc",
      bun: "bun remove -g drc",
      yarn: "yarn global remove drc",
      brew: "brew uninstall drc",
      choco: "choco uninstall drc",
      scoop: "scoop uninstall drc",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result =
        method === "choco"
          ? await $`echo Y | choco uninstall drc -y -r`.quiet().nothrow()
  ...
  UI.empty()
  prompts.log.success("Thank you for using DrC!")
}
