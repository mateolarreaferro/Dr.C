...
export namespace SessionPrompt {
  ...
  export const loop = fn(LoopInput, async (input) => {
    ...
    while (true) {
      ...
      if (agent.name === "csound") {
        ...
        // Resolve through workspace (reads workspace copy if active, original otherwise)
        if (activeCsdPath) {
          activeCsdPath = SessionWorkspace.resolve(sessionID, activeCsdPath)
        }

        // Read CSD content once (shared by rewriter + locked constraints)
        let csdContent: string | undefined
        if (activeCsdPath) {
          try {
            csdContent = await Bun.file(activeCsdPath).text()
          } catch { /* empty */ }
        }
        ...
        const [multiTierResult, lockedConstraints] = await Promise.all([
          ...
          // Locked param constraints (pure CPU, fast)
      ...
      if (agent.name === "csound" && retrievedChunkIDs.length > 0) {
        try {
          const toolParts = (await MessageV2.filterCompacted(MessageV2.stream(sessionID)))
            .flatMap((m) => m.parts)
            .filter((p) => p.type === "tool" && "state" in p && p.state.status === "completed") as any[]
          const toolResults = toolParts.map((p: any) => ({
            tool: p.tool as string,
            success: !p.state.error,
            metadata: "state" in p ? (p.state as any).metadata : undefined,
          }))
          const signal = RetrievalFeedback.deriveSignal(toolResults)
          await RetrievalFeedback.record(sessionID, "", retrievedChunkIDs, signal)
        } catch {
  ...
  async function insertReminders(input: { messages: MessageV2.WithParts[]; agent: Agent.Info; session: Session.Info }) {
    ...
    if (!Flag.DRC_EXPERIMENTAL_PLAN_MODE) {
