...
<CsInstruments>
; flngnote.orc (in the noteffct subdirectory on the CD-ROM)

;instr 152 - general wavetable wind instrument with flanging
...
instr 152                                                   ; general wind instrument with flanging
; parameters
...
; p11 flanging depth ("amount" of flanging): 0=minimum amount, 1=maximum
; p12 minimum delay time; range: .001-.008" (must be greater than zero)
; p13 maximum delay time, e.g. .003-.016" (must be greater than minimum delay time)
; p14 flanging rate (rate of change between minimum and maximum delay times)
;         set so that flanger sweeps up and down every few seconds
;         typical values range from .25 (4" sweeps) to .5 (2" sweeps)
; p15 % of feedback, typical values vary from .1 to .5
;         0=no feedback, values close to 1.0 produce video game effects
; p16 flanging on-off switch
...
imindt    =       p12
imaxdt    =       p13
imindt    =       (p12 > p13 ? p13 : imindt)
imaxdt    =       (p12 > p13 ? p12 : imaxdt)
...
<CsScore>
; flngnote.sco (in the noteffct subdirectory on the CD-ROM)
; note-specific flanging example (with flute)
...
;    start   dur amp  Hertz   vibr  att   dec   br in  depth mindt maxdt rate  feed sw
i152  1.000  5   4000 558.080 1.000 0.230 0.100  5  2   .5   .005  0.008 0.333 0.5   1   
i152  5.000  9.5 5000 279.040 1.000 0.230 0.100  5  2   .5   .004  0.008 0.15  0.7   1
i152  7.000  5   5000 837.120 1.000 0.230 0.100  5  2   .5   .0045 0.008 0.23  0.4   1
i152  9.000  6   5000 697.600 1.000 0.230 0.100  5  2   .5   .0055 0.009 0.27  0.8   1
i152 13.100  3   5000 976.640 1.000 0.130 0.100  5  2   .5   .004  0.008 0.30  0.2   0

;reverb---------------------------------------------------------------------------------
;p1    p2      p3   p4        p5        p6
;      start   dur  revtime   %reverb   gain
i194   0.5     18   1.5       .05       1.0      ;  reverb blurs the flanging effect
end
</CsScore>
</CsoundSynthesizer>
<bsbPanel>
