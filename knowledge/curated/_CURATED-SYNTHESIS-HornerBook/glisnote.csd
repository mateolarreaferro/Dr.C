...
<CsInstruments>
; glisnote.orc (in the season subdirectory on the CD-ROM)
...
instr 150                                            ; general wavetable wind instrument with glissando
; parameters
...
; p11 2nd pitch for glissando in Hertz
; p12 percent duration to wait before glissando
; p13 percent duration for glissando
...
ifreq2    =       p11                                       ; 2nd pitch for glissando in Hertz
p12       =       (p12 <= 0 ? .01 : p12)                    ; keep values > 0
p13       =       (p13 <= 0 ? .01 : p13)                    ; keep values > 0
iwait     =       p12 * idur                                ; percent duration to wait before glissando
igliss    =       p13 * idur                                ; percent duration for glissando
...
kfreq     expseg  ifreq,iwait,ifreq,igliss,ifreq2,1,ifreq2  ; tuning envelope for glissando
kfreq     =       kfreq + kvib
...
<CsScore>
; glisnote.sco  (in the season subdirectory on the CD-ROM)
...
;     start   dur    amp   Hertz   vibr   att    dec    br in Hz2   wait gliss ; freqRatios
i150   1.000  5.100  1000  220.00  1.000  0.060  0.150  9  3  247.5 .2   .5    ; 1/1 - 9/8
i150   7.000  5.100  1000  275.00  1.000  0.060  0.150  9  3  550   .2   .5    ; 5/4 - 5/2
i150  13.000  2.100  1000  330.00  1.000  0.060  0.150  9  3  110   .2   .5    ; 3/2 - 1/1

;reverb------------------------------------------------------------------
;p1   p2     p3    p4       p5      p6
;     start  dur   revtime  %revrb  gain
i194  0.5    15.7  1.0      .05     1.0
end
</CsScore>
</CsoundSynthesizer>
<bsbPanel>
